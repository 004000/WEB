...
כדי למנוע התמלאות מסד הנתונים (Redis/kvrocks), המערכת יכולה לגבות ולמחוק לצמיתות הודעות ישנות. יש שני מנגנונים משלימים:

1. **ניקוי לפי גיל** — רץ אוטומטית פעם ביום, מוחק הודעות מעל תקופה מוגדרת (ברירת מחדל: 90 יום).
2. **ניקוי חירום לפי תפוסה** — רץ אוטומטית כל שעה, בודק את אחוז התפוסה של מסד הנתונים, ואם עוברים סף מוגדר
   (ברירת מחדל: 80%) — מוחק את ההודעות **הישנות ביותר** שיש, ללא קשר לגיל שלהן, עד שהתפוסה יורדת מתחת לסף.
   זה מגן על ערוצים עם קצב פרסום גבוה, שעלולים להגיע לתקרת האחסון תוך פחות מ-90 יום.

שני המנגנונים ניתנים גם להפעלה ידנית מפאנל הניהול, בלשונית "אחסון וגיבוי".
